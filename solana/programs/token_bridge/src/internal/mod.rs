...
pub fn cpi_send_call<'info>(
    ...
    portal_cpi::send_call(cpi_ctx, CallType::Call, REMOTE_BRIDGE, gas_limit, data)?;

    Ok(())
}

pub fn cpi_send_call_with_eth<'info>(
    portal: &Program<'info, Portal>,
    accounts: portal_cpi::accounts::SendCallWithEth<'info>,
    authority_bump: u8,
    gas_limit: u64,
    value: u64,
    data: Vec<u8>,
