...
describe("receiver", () => {
  ...
  describe("Relayed bridge transaction", () => {
    ...
    let message: Buffer;
    let serializedMessengerPayload: Buffer;
    let serializedBridgePayload: Buffer;
    ...
    before(async () => {
      ...
      serializedBridgePayload = Buffer.concat([
        localTokenBuffer,
        remoteTokenBuffer,
        fromBuffer,
        toBuffer,
      ...
      // Encode bridge message into Vec<u8>
      let serializedIxParam = Buffer.alloc(0);

      // Program ID
      serializedIxParam = Buffer.concat([
        serializedIxParam,
        expectedBridgePubkey.toBuffer(),
      ]);
      ...
      const accountsLen = Buffer.alloc(4);
      accountsLen.writeUInt32LE(transferAccounts.length, 0);
      serializedIxParam = Buffer.concat([serializedIxParam, accountsLen]);
      ...
      // Length of the data vector (u32 LE)
      const dataLen = Buffer.alloc(4);
      dataLen.writeUInt32LE(serializedBridgePayload.length, 0);
      serializedIxParam = Buffer.concat([serializedIxParam, dataLen]);
      serializedIxParam = Buffer.concat([
        serializedIxParam,
